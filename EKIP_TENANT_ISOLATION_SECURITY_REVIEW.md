...
2. **CONFIRMED LIVE, still open (2026-08-18, Batch 4.6 migration investigation): the database role the application connects as is a table-owner role that bypasses RLS.** Direct `pg_roles` introspection against the real Neon development database shows `neondb_owner` (what `DATABASE_URL` actually connects as) has `rolbypassrls = true` — every RLS policy from section 3 is a live no-op against this database today, exactly the failure mode this recommendation warned about. A separate, correctly-provisioned role (`ekip_app`, `NOSUPERUSER`/`NOBYPASSRLS`) already exists in that same database — created by an unmerged branch's migration, never wired into `Settings`/`app.database.session` on `main` — see `docs/operations/migration-recovery.md` for the full provenance. **Fixing this requires**: (a) deciding on a real least-privilege application role for `main`'s own migration history (reusing or re-authoring `ekip_app`'s definition), (b) rotating the application's connection credential to that role everywhere `DATABASE_URL` is configured, and (c) re-verifying every RLS-dependent code path still works under the narrower role's actual grants — not yet done anywhere.
3. **NEW, CONFIRMED LIVE (2026-08-18, Batch 4.6): `app.core.incidents.service.get_incident`, `list_incidents`, and `get_timeline` have no permission check at all, only `_ensure_same_organization`** (`_get_owned_incident`, which backs all three, checks solely `organization_id` equality — see `service.py`'s own `_get_owned_incident`). Any identity in the organization — including one with zero role assignments — can read every incident's full detail and timeline across every project, unlike every write path in the same module (`require_project_permission(actor, ..., "incident:write")`). This is not a new finding invented here: an unmerged branch's migration (`b6e9c2a4f7d1`) documents it as an already-confirmed prior audit finding ("2026-08 audit 'H4'") and seeds the `incident:read`/`postmortem:read` permissions its own (also unmerged) code fix requires — but that code fix never reached `main`. Closing this requires porting or re-implementing the `require_project_permission(actor, project_id, "incident:read")` gate on the three read paths above, plus seeding the permission and backfilling it to every existing role (the same backward-compatibility approach `b6e9c2a4f7d1` already took) so existing users aren't instantly locked out the moment the check ships.
4. **Also still open: run the full RLS migration + bypass functions against a real Postgres instance at least once** (a staging environment, or even a local `docker-compose` Postgres) before treating this as production-ready — every piece of Milestone 10's RLS work was written and unit-tested without ever executing against a live database this session, per the recurring sandbox-unavailability disclosure throughout this milestone.
5. As a smaller, independent follow-up: extend this review's service-layer ownership-check verification to `core/auth/service.py` and `core/users/service.py`'s remaining PK-based lookups, closing the one disclosed gap in section 2 above.
6. Re-run this same review (or an automated equivalent — an import-linter-style or custom lint rule flagging any `repository.py` `select()` against a known tenant-owned model with no `organization_id` in its `.where()` clause) after any future milestone that adds new tenant-owned tables, so this doesn't have to be a fully manual re-audit every time. The same lint rule should also flag any new bare-PK/bare-hash lookup against an RLS-protected table that doesn't go through an already-established `set_tenant_context` chokepoint, so a fifth chicken-and-egg case (like the refresh-token one found and closed in this pass) doesn't go unnoticed in a future milestone.
