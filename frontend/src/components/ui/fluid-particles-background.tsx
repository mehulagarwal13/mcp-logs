...
interface CyberBackgroundProps {
   ...
   * behind everything, no forced height / centering / opaque fill) instead
   * of the default self-contained `h-screen` hero wrapper. Use this when
   * the canvas should sit behind a scrolling app shell.
   */
...
export const FluidParticlesBackground = ({
  ...
  // Read as primitives so a fresh `{ min, max }` object literal on each
  // render doesn't re-trigger the effect (and reset every particle).
  const sizeMin = particleSize?.min ?? 0.5;
  const sizeMax = particleSize?.max ?? 2;
  // Perlin permutation table is expensive and immutable — build it once,
  // not on every render (the original recreated it each render, which also
  // made the effect below re-subscribe endlessly).
  ...
  useEffect(() => {
    ...
    if (prefersReducedMotion) {
      renderFrame(); // paint a single static frame
