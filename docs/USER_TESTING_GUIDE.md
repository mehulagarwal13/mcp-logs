> **⚠️ Read this first, and re-read it after every section.** This guide was written by directly reading the EKIP source code on 2026-08-06 — every claim below is cross-referenced to a real file, and every gap is called out explicitly rather than glossed over. Two of the top-level docs in this repo (`docs/PROJECT_STATUS.md`, `docs/PROJECT_STRUCTURE.md`) are **stale** — they describe an early "Phase 1, no implementation yet" state, while the actual `app/` tree is fully built through Milestone 10. If anything here ever conflicts with another doc in the repo, trust the code (and this guide), not the older planning docs.
...
| 28 | Project-scoped permission overrides | ⚠️ | n/a | `Identity.project_permissions` field exists; nothing populates it | Always falls back to org-level permissions |
...
- `app/retrieval/qdrant/` — empty placeholder package; `Settings.default_vector_backend` setting has zero effect regardless of value.
- `Identity.project_permissions` — accepted as a parameter everywhere, populated nowhere.
- `core.auth.service.revoke_all_sessions` — implemented, zero REST/MCP callers.
- `core.tenancy.service.{create_organization, get_organization, list_organizations, configure_sso, list_projects, create_project, create_access_rule, list_access_rules, deactivate_access_rule, create_invitation, list_invitations, revoke_invitation, accept_invitation}` — fully implemented, **zero REST/MCP surface**. `accept_invitation` is invoked internally during SSO callback, so it's reachable indirectly; the rest are only reachable via direct Python import.

### 3.4 Stale documentation to distrust

`docs/PROJECT_STATUS.md` and `docs/PROJECT_STRUCTURE.md` describe the project as barely started — ignore both. `EKIP_STRATEGIC_ANALYSIS.md` claims `shared/security` is a pass-through stub — **false**, it's real AES-256-GCM encryption; only SSO client-secret resolution (a different function) is still a stub. Several connector docstrings still say credential decryption "is not yet built" — also false as of Milestone 10; verify against `app/ingestion/service.py` (the `decrypt_secret` call) instead of trusting the connector's own comment.
