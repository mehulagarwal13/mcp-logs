...
**Superseding note (added later, `core/tenancy` now built):** the "not yet populated" statement above is no longer current. `core/users/service.py`'s `resolve_identity()` now populates `project_permissions` via `repository.get_project_permission_map()` (joins `project_memberships -> projects -> role_permissions -> permissions`, scoped by `projects.organization_id` since `project_memberships` carries no `organization_id` column of its own). This was purely additive — no signature or behavior change to `has_permission`/`authorize`, which already supported `project_id` per this decision. Identities constructed outside `resolve_identity` (e.g. `Identity.for_agent`) still default to an empty mapping and fall back to org-level `permissions`, which remains correct per the original decision above. `shared/schemas/identity.py`'s docstring was updated to match at the same time this note was added.
...
- **Confidence-score formula and threshold** — will be decided empirically once real retrieval data exists; placeholder logic (default `0.6`, configurable via `Settings.confidence_threshold`) ships first.
  - *Update (2026-09-02) — two signal-quality fixes applied, threshold moved provisionally, formula still open:* an audit found (1) `source_count` scored a fully-correct single-document answer at 0.2 (linear `n/5`), structurally penalising the common single-source case, and (2) `top_similarity` was seeded from the fused RRF score and pinned at a near-constant ~0.5 for virtually every query, discriminating nothing. `app/agents/confidence.py` now maps `source_count` through a diminishing-returns curve (1 source → 0.70) and `top_similarity` is the top *dense* cosine similarity (via `retrieval.service.search_with_signals`), matching this doc's and `AGENT_WORKFLOWS.md` §2.2's original "best chunk's vector similarity score" intent. Both shift the score distribution; working the 2026-08-13 eval's per-category behavior through the new formula projects no-information ~0.17, the two former clear-answer misroutes ~0.51–0.59, clear-answer strong ~0.80, so `Settings.confidence_threshold` was lowered `0.6 → 0.5` **provisionally** (a projection, not a fresh sweep). `scripts/eval_confidence.py --compare-to scripts/eval_confidence_report.json` must still be re-run against live data to confirm it; `scripts/eval_confidence_report.json` and the (old) run details in the `confidence_threshold` comment are stale until then.
